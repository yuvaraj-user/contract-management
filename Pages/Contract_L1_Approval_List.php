		...
		$(document).ready(function() {
 		...
 		function get_contract_pending_list(action_status){
            $.ajax({
                url: 'Common_Ajax.php',
                type: 'POST',
                data: { Action : 'Contract_L1_Approval_List',status : action_status },
                dataType: "json",
                beforeSend: function(){
                    showLoader();
                },
                        ...
                        for(i in response.data) {   
                            ...
                            html += `<tr>
                                ...
                                <td>
                                    ...
                                    <a href="Contract_L1_Approve.php?id=${ encodedId }&action=${btoa('view')}" class="text-inverse pr-10 tbl_action_view" title="View" data-toggle="tooltip" data-original-title="Edit">
                                		<i class="zmdi zmdi-eye txt-primary"></i>
                                	</a>`;
                            
                               if(action_status == 'Pending') {
                                  html += `<a href="Contract_L1_Approve.php?id=${ encodedId }" class="text-inverse pr-10" title="Approve" data-toggle="tooltip" data-original-title="Edit">
                                  		<button class="btn btn-sm btn-warning"><i class="fa-solid fa-check"></i> Approve</button>
                                	</a>`;
                                }

                           		if(action_status == 'Completed' && response.data[i].Form_Status == 'Approved') {
