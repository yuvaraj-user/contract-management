 ...
 * 
 */
class Contract_L1_Approval_Details
{
	public $conn;
	...
	public function Contract_L1_Approval_List($request)
	{
		$response['status']  = 422;
		...
		$Qry     = "SELECT *,FORMAT(Contract_Period_From,'dd-MM-yyyy') as Contract_Period_From_Date,FORMAT(Contract_Period_To,'dd-MM-yyyy') as Contract_Period_To_Date FROM Contract_Portal_Vendor_Registration WHERE Form_Status != 'Deleted' AND L1_Emp_Id = '".$Emp_Id."'";
		...
		if(isset($request['status']) && $request['status'] == 'Pending') {
			$Qry .= " AND Form_Status = 'Submitted' AND Contract_Approval_Submission = 1";
		}
	...
	public function Contract_L1_Status_Update($request)
	{
		...
		if(isset($_SESSION['EmpID']) && $_SESSION['EmpID'] != '') {
	  		$Emp_Id = $_SESSION['EmpID'];
	  		$Id     = $request['id']; 
	  		$CON_ID = $request['cont_id']; 	
	  		$Form_Status = $request['contract_action_status']; 
